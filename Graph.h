...
public:
	...
	int MaxFlow(std::string from, std::string to)
	{
		...
		return flow; //BUG// STUB WAS NOT REMOVED
	}
	...
	int FindMinimalCover(Graph* tAnswer)
	{
		...
		do
		{
			...
			if (found)
			{
				tAnswer->Connect(edge->_from->_data, edge->_to->_data);
				totalWeight += edge->_weight;
				edge->_from->_processed = true;
				edge->_to->_processed = true;
			}
		} while (found);
