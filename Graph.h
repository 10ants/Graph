...
public:
    ...
    {// "Overloading" like this prevents copypaste, and the BestDistance method needs weight
        Connect(from, to, 1);
    }
    ...
    int MaxFlow(std::string from, std::string to)
    {
        ...
        do
        {
            found = working.BestDistance(from, to) < INT_MAX; //BUG// Should be int max since looking for something negative would make no sense
            if (found)
            {
                int smallest = INT_MAX;  //BUG// Again should be int max
                ...
                for (auto edge : path)
                {
                    working.Connect(edge->_to->_data, edge->_from->_data, smallest);
                    edge->_weight -= smallest; //BUG// Should be measured against smallest
                }
                flow += smallest;
            }
        } while (found);

        return flow; //BUG// Stub not removed
    }
    ...
    int FindMinimalCover(Graph* tAnswer)
    {
        ...
        {
            found = false;
            int lowest = INT_MAX; //BUG// Should be INT_MAX instead of 0, since we have to compare less than lowest in our if loop.
            ...
            for (auto one : _mainData)
            {
                ...
                {
                    if (!edge->_from->_processed && edge->_weight < lowest) //BUG// We want _from not _to
                    {
                        found = true;
                        lowest = edge->_weight;
                        lowestEdge = edge;
                ...
                {
                    if (!edge->_to->_processed && edge->_weight < lowest)//BUG// We want _to not _from
                    {
                        found = true;
                        lowest = edge->_weight;
                        lowestEdge = edge;
                ...
                if (found)
                {
                    ...
                    totalWeight += lowestEdge->_weight;
        ...
        return totalWeight; //BUG// Stub not removed
    }
